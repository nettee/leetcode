...
public:
    ...
    int mergesort(vector<int>& a, vector<int>& aux, int start, int end) {
        ...
        int inner = countReverse(a, start, mid, end);
        merge(a, aux, start, mid, end);
        return left + right + inner;
    }
    
    int countReverse(vector<int>& a, int start, int mid, int end) {
        int sum = 0;
        int count = 0;
    ...
    void merge(vector<int>& a, vector<int>& aux, int start, int mid, int end) {
        int i = start;
        int j = mid;
        int k = start;
        while (i < mid || j < end) {
